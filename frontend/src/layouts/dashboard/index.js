...
//         // Filter transactions by selected firm
//         const filteredTransactions = selectedFirm
//           ? transactions.filter(tx => tx.reportEntry.fromFirmID === selectedFirm.value || tx.reportEntry.toFirmID === selectedFirm.value)
//           : transactions;
//         console.log('f',filteredTransactions)
//         // Extract data for chart
//         const labels = filteredTransactions.map(tx => new Date(tx.reportEntry.transactionDate).toLocaleDateString());
//         const fromAmounts = filteredTransactions.map(tx => parseFloat(tx.reportEntry.Amount));
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Filter transactions by selected firm and current month
        const filteredTransactions = transactions.filter(tx => {
          const transactionDate = new Date(tx.reportEntry.transactionDate);
          const txMonth = transactionDate.getMonth();
          const txYear = transactionDate.getFullYear();
          
          return (selectedFirm ? 
            (tx.reportEntry.fromFirmID === selectedFirm.value || tx.reportEntry.toFirmID === selectedFirm.value) 
            : true) && 
            (txMonth === currentMonth && txYear === currentYear); // Filter for current month
        });
