...
import PaymentHistory from "layouts/payment/components/PaymentHistory";
...
function Transactions() {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      if (selectedFirm) {
        try {
          const transactionsData = await getTransactions(selectedFirm.value, 2); // Fixed transaction type as 1
          ...
          {error ? (
            ...
            transactions.map((transaction) => (
              <PaymentHistory
                ...
                name={`${transaction.ToFirmName} (${transaction.ToLedgerName}) From ${transaction.FromFirmName} (${transaction.FromLedgerName})`} // Display from/to Firm and Ledger names
